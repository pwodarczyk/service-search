...
import com.rr.generalservicesapplication.userJobsList.FrUserJobsAdaptor;
...
public class Fr_Employee_Jobs extends Fragment {

	public Fr_Employee_Jobs(){}
	// Declare Variables
	ListView listview;
	List<ParseObject> ob;
	ProgressDialog mProgressDialog;
	FrEmployeeJobsAdaptor adapter;
	ParseFile image;
	String emailll;
		...
		try {
			...
			try {
				for (int i = 0; i < JA.length(); i++) {
					...
					adapter = new FrEmployeeJobsAdaptor(getActivity(), Utils.UserJobList);
					listview.setAdapter(adapter);
